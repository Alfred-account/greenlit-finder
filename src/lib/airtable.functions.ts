...
function getConfig() {
  ...
  const table =
    process.env.AIRTABLE_TABLE_NAME ?? process.env.VITE_AIRTABLE_TABLE_NAME ?? "Opportunities";
  ...
  async (): Promise<{
    ...
    if (!config) {
      return {
        ...
        source: "sample",
        error:
          "Airtable не настроен: отсутствуют переменные окружения AIRTABLE_API_KEY и/или AIRTABLE_BASE_ID.",
      };
    }
    ...
    try {
      ...
      if (res.status === 422) {
        ...
        console.error(
          `[airtable] Filtered request failed [422] ${url}: ${first} — retrying without filterByFormula`,
        );
